...
;// CONCATENATED MODULE: ./src/js/libs/datepicker.js




const mobile = isMobile.any();
const mobileMediaQuiry = window.matchMedia('(max-width: 479.98px)');


const datepickerRange = new index_es('[data-date-range]', {
  range: true,
  multipleDatesSeparator: ' - ',
  dynamicRange: true,
  isMobile: mobileMediaQuiry.matches && mobile ? true : false,
  position: 'top center'
});
...
function documentActions(e) {
  ...
  if (targetElement.closest('.hero__open-form')) {
    document.documentElement.classList.add('_show-form', "lock");
    targetElement.setAttribute("aria-expanded", "true");
  }
  if (targetElement.closest('.program-form__close')) {
    const buttonOpen = document.querySelector('.hero__open-form');
    document.documentElement.classList.remove('_show-form', "lock");
